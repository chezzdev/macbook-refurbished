...
в EUR с двумя знаками после запятой. Под каждой ES-ценой отдельно выводятся
стоимость без IVA и включённый IVA по общей ставке 21%. База рассчитывается как
`цена Apple / 1,21`, затем обе части округляются до евроцента.

[Agencia Tributaria указывает общую ставку IVA 21%](https://sede.agenciatributaria.gob.es/Sede/iva/calculo-iva-repercutido-clientes/tipos-impositivos-iva.html).
Вычет или возврат зависит от статуса покупателя, счёта и способа получения
заказа. [Apple описывает отдельные условия для бизнеса и DIVA](https://www.apple.com/es/shop/help/payments),
включая самовывоз в Apple Store для подходящих tax-free заказов. Показанная
сумма IVA является арифметической разбивкой цены, а не обещанием возврата.
